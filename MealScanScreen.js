...
function MealScanScreen(props) {
  ...
  const fetchMealDetails = useCallback((tagin, mealType) => {
    ...
    api.get('getMemberActivity', {
      ...
      .then(async (json) => {
        ...
        console.log(data);
        setError(false);
        // Get fresh scannerMemberId inside the promise chain
        const currentScannerId = await AsyncStorage.getItem('memberId');
        console.log('Current scannerMemberId in promise:', currentScannerId);
        setScannerMemberId(currentScannerId);
  ...
  const addMemberActivity = async (meal, data) => {
    if (data && data.memberActivityDetails) {
      const formattedLocation = getLocation();
      // Get scannerMemberId directly from AsyncStorage
      const currentScannerId = await AsyncStorage.getItem('memberId');
      console.log('Using scannerMemberId in addMemberActivity:', currentScannerId);
  ...
  const removeMemberActivity = async (meal) => {
    const formattedLocation = getLocation();
    if (data && data.memberActivityDetails) {
      // Get scannerMemberId directly from AsyncStorage
      const currentScannerId = await AsyncStorage.getItem('memberId');
      console.log('Using scannerMemberId in removeMemberActivity:', currentScannerId);
    ...
    return (
      <View>
        ...
        <View style={styles.row}>
          ...
          <View>
            <ButtonSlim onPress={() => {
              setRefreshing(false); setIsAddingMeal(true);
              addMemberActivity(currentMeal, data);
            }} text="ADD MEAL" colors={['#2EB62C', '#83D475']} disabled={isAddingMeal} />
          </View>
          <View>
            <ButtonSlim onPress={() => {
              navigation.navigate('Home'); // Navigate to the Home screen
